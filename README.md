...
* `Auto-create RSVP` - Plugin Human-readable name, e.g. "Sample Extension"
* `Autocreate_RSVP` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `auto-create-rsvp` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `auto_create_rsvp` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
